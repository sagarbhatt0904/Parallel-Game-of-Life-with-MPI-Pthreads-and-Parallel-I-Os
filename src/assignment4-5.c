 ...
 *  rows are ghosted.
 */
void play_gol(int start, int end);
...
//    int i = 0;
  ...
  rc = pthread_barrier_init(&barr, NULL, NUM_THREADS);
  if (rc != 0)
  {
...
// Insert your code
  if (1)
  {
    ...
    univNew  = (celltype **)malloc(sizeof(celltype *) * (nrows + 2));
    univNew[0] = (celltype *)malloc(sizeof(celltype) * ncols * (nrows + 2));
    ...
    for (int t = 0; t < NUM_THREADS; t++)
    {
      ...
      rc = pthread_create(&threads[t], NULL, &play_gol_thread, (void*) &tid[t]);
      if (rc)
      {
...
//   size_t nicols = ncols / CELLS_PER_PIXEL;
//   size_t ci, cj, ii, ij;
//   for (int ii = 0; ii < nirows; ++ii)
// {
//     for (int ij = 0; ij < nicols; ++ij)
// {
//       img[ii][ij] = 0;
